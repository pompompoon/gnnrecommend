...
ユーザーごとに嗜好プロファイル（好みカテゴリ・ブランド・価格帯）を割り当て、
...
from data.categories import (
    CATEGORY_HIERARCHY, BRANDS, COLORS, SEASONS, PRICE_RANGES, ITEM_TEMPLATES,
)
...
def generate_all(config: Config) -> None:
    ...
    item_rows = _generate_and_insert_items(db, config.data.n_items, subcat_map, brand_map, rng)
    _generate_and_insert_users(db, config.data.n_users, rng)
    _generate_interactions(db, config.data, item_rows, rng)
    _print_stats(db)

def _insert_categories(db):
    cat_map, subcat_map = {}, {}
    for sort_idx, (cat_name, info) in enumerate(CATEGORY_HIERARCHY.items()):
        db.execute("INSERT INTO categories (name, name_en, sort_order) VALUES (%s, %s, %s) ON CONFLICT (name) DO NOTHING", (cat_name, info["name_en"], sort_idx))
        ...
        for sub_idx, (sub_name, sub_en) in enumerate(info["subcategories"]):
            db.execute("INSERT INTO subcategories (category_id, name, name_en, sort_order) VALUES (%s, %s, %s, %s) ON CONFLICT (category_id, name) DO NOTHING", (cat_id, sub_name, sub_en, sub_idx))
            row = db.fetch_one("SELECT subcategory_id FROM subcategories WHERE category_id=%s AND name=%s", (cat_id, sub_name))
            subcat_map[sub_name] = row["subcategory_id"]
    print(f"  📁 カテゴリ {len(cat_map)} / サブカテゴリ {len(subcat_map)}")
    return cat_map, subcat_map

def _insert_brands(db):
    brand_map = {}
    for name in BRANDS:
        db.execute("INSERT INTO brands (name) VALUES (%s) ON CONFLICT (name) DO NOTHING", (name,))
        row = db.fetch_one("SELECT brand_id FROM brands WHERE name=%s", (name,))
        brand_map[name] = row["brand_id"]
    print(f"  🏷️  ブランド {len(brand_map)}")
    return brand_map

def _generate_and_insert_items(db, n, subcat_map, brand_map, rng):
    ...
    for _ in range(n):
        ...
        name = f"{name} ({color})"
        lo, hi = PRICE_RANGES.get(sub_name, (5000, 30000))
        price = int(np.clip(rng.normal((lo + hi) / 2, (hi - lo) / 4), lo, hi))
        price = round(price, -2)
        is_on_sale = bool(rng.random() < 0.15)
        rows.append(dict(name=name, subcategory_id=subcat_map[sub_name], brand_id=brand_map[brand_name], price=price, color=color, season=season, is_on_sale=is_on_sale, description=f"{brand_name} {season}。{color}の{sub_name}。"))
    values = [(r["name"], r["subcategory_id"], r["brand_id"], r["price"], r["color"], r["season"], r["is_on_sale"], r["description"]) for r in rows]
    db.execute_values("INSERT INTO items (name, subcategory_id, brand_id, price, color, season, is_on_sale, description) VALUES %s", values)
...
def _generate_and_insert_users(db, n, rng):
    prefectures = ["東京都", "神奈川県", "大阪府", "愛知県", "埼玉県", "千葉県", "兵庫県", "北海道", "福岡県", "京都府", "静岡県", "広島県", "宮城県"]
    values = []
    for _ in range(n):
        values.append((fake.unique.user_name(), int(rng.integers(18, 60)), str(rng.choice(["male", "female", "other"], p=[0.50, 0.45, 0.05])), str(rng.choice(prefectures))))
    db.execute_values("INSERT INTO users (username, age, gender, prefecture) VALUES %s", values)
    print(f"  👤 ユーザー {n}")

def _build_user_profiles(user_ids, cat_ids, brand_ids, rng):
    profiles = {}
    for uid in user_ids:
...
def _affinity_scores(profile, items, max_price):
    n = len(items)
...
def _generate_interactions(db, data_cfg, item_rows, rng):
    users = db.fetch_all("SELECT user_id FROM users ORDER BY user_id")
    user_ids = [u["user_id"] for u in users]
    items_full = db.fetch_all("SELECT i.item_id, i.subcategory_id, i.brand_id, i.price, s.category_id FROM items i JOIN subcategories s ON i.subcategory_id = s.subcategory_id ORDER BY i.item_id")
    ...
    db.execute_values("INSERT INTO views (user_id, item_id, duration_sec, viewed_at) VALUES %s", view_values)
    ...
    db.execute_values("INSERT INTO purchases (user_id, item_id, quantity, purchased_at) VALUES %s", purchase_values)
    ...
    while len(fav_set) < data_cfg.n_favorites and attempts < data_cfg.n_favorites * 5:
        ...
        fav_set.add((uid, iid))
        attempts += 1
    fav_values = [(uid, iid, base_time + timedelta(seconds=int(rng.integers(0, span_sec)))) for uid, iid in fav_set]
    db.execute_values("INSERT INTO favorites (user_id, item_id, favorited_at) VALUES %s", fav_values)

def _print_stats(db):
    tables = ["categories", "subcategories", "brands", "items", "users", "purchases", "views", "favorites"]
    print("\n✅ データ生成完了:")
    for t in tables:
